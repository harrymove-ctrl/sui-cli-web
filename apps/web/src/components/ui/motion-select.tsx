// Animated select - adapted from beui.dev/components/motion/select.
//
// Two deltas from the source: Tailwind v4's `border-(--token)` shorthand is
// rewritten for this project's v3 setup, and each option is a `div` rather than
// an `li` (the listbox is a `div`, so a bare `li` would be invalid markup - the
// `role="option"` is what assistive tech reads either way).
...
export function Select({
  ...
  useEffect(() => {
    ...
    const onPointer = (e: PointerEvent) => {
      ...
      // The panel is portaled to <body>, so it is NOT inside rootRef - without
      // this, pressing an option would close the menu on pointerdown and the
      // click would never land on the option.
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  // Viewport coords of the trigger, so the portaled panel can track it.
  ...
  // Track the trigger's box while open. The panel is portaled to <body>, so it
  // no longer inherits the trigger's position - it has to be told.
  useLayoutEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    <motion.div
      ...
      style={{
        ...
        // Fixed + portaled: any ancestor with `overflow-hidden` would clip the
        // panel, and any ancestor with `backdrop-filter`/`transform` starts a
        // stacking context that traps its z-index under later siblings.
        position: 'fixed',
        left: anchor.left,
        width: anchor.width,
        ...(isTop
      ...
      className={cn('z-50 rounded-xl border border-border bg-background shadow-lg', className)}
    >
      <motion.div
        ref={innerRef}
