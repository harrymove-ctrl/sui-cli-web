...
interface StreamingTextProps extends HTMLAttributes<HTMLSpanElement> {
  ...
  delay?: number;
  /** How long a word wears the gradient before settling into the
   *  theme foreground, in milliseconds. */
  settleDelay?: number;
  /** Show a small pulsing gradient dot riding the stream head. */
  showCursor?: boolean;
  /** Tailwind classes for the ink gradient a word wears while it's fresh.
   *  Defaults to the blue ribbon; override to match a surrounding accent. */
  gradientClassName?: string;
  /** Tailwind classes for the pulsing cursor dot. Pair with gradientClassName. */
 ...
 *  of color always marks what just arrived. */
const gradientText =
...
const cursorDot =
  'bg-gradient-to-br from-sky-400 to-indigo-500 dark:from-sky-300 dark:to-indigo-400 shadow-[0_0_0.7em_rgba(56,189,248,0.55)] dark:shadow-[0_0_0.7em_rgba(125,211,252,0.45)]';

/** Consecutive words sample a 3x-wide version of the gradient at stepped
 *  offsets that ping-pong across GRADIENT_PHASES positions, so the color
 *  trail reads as one continuous ribbon flowing over several words instead
 *  of every word restarting at the first stop. Static paint — the offset
 *  never animates, so it costs nothing per frame. */
 ...
 *  many words are colored at once. */
...
const StreamedWord = memo(function StreamedWord({
  ...
  return (
    ...
    >
      ...
      <motion.span
        animate={{ opacity: 0 }}
        className={cn('col-start-1 row-start-1', gradientClassName)}
        initial={{ opacity: 1 }}
        style={{
  ...
  (
    {
      ...
      settleDelay = 300,
      showCursor = true,
      gradientClassName = gradientText,
      cursorClassName = cursorDot,
      onComplete,
      ...props
    },
    ref
