// Command palette - adapted from beui.dev/components/blocks/command-palette.
//
// Note the app already binds ⌘K globally to the QuickSwitcher (see MainLayout),
// so any instance of this must pick a different `shortcut`.
...
export function CommandPalette({
  ...
  // Portal target only exists client-side; render nothing during hydration.
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape' && open) {
        // Stop here: the app closes/navigates on bare Escape too, and both
        // firing would shut the palette *and* bounce you to the dashboard.
        e.preventDefault();
        e.stopPropagation();
        setOpen(false);
      }
  ...
  const runItem = (it: CommandItem | undefined) => {
  ...
  return createPortal(
    ...
    >
      <motion.button
        ...
        className={cn(
          'absolute inset-0 bg-background/40 [backdrop-filter:blur(12px)_saturate(140%)] [-webkit-backdrop-filter:blur(12px)_saturate(140%)]',
                        ...
                        {isActive ? (
                          <motion.span
                            ...
                            transition={
                              reduce
                                ? { duration: 0 }
                                : // Tracks rapid arrow-key navigation - keep it tight
                                  // so it never lags the active row.
